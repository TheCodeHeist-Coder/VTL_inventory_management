...
export default function DistrictsPage() {
    ...
    return (
        <>
            ...
            {/* Filters */}
            <div className="bg-white border border-brand-200 rounded-xl shadow-sm overflow-hidden mb-6"><div className="p-6" style={{ padding: '16px 24px' }}><div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {filteredDistricts.map(d => (
                ...
                <div key={d.id} className="bg-white border border-brand-200 rounded-xl shadow-xl overflow-hidden mb-6">
            ...
            {showDistrictModal && (
                <div className="fixed inset-0 bg-brand-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={(e: any) => e.target === e.currentTarget && setShowDistrictModal(false)}>
                    <div className="bg-white border border-brand-200 rounded-2xl w-full max-w-xl shadow-xl p-6">
                        <h2 className="text-lg font-bold text-brand-900 mb-5"> New District</h2>
                        <form onSubmit={createDistrict}>
                            <div className="mb-5"><label className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-1.5">State</label><select className="w-full px-4 py-2.5 bg-white border border-brand-200 rounded-lg text-sm cursor-pointer" value={districtForm.stateId} onChange={e => setDistrictForm({ ...districtForm, stateId: e.target.value })}><option value="">— None —</option>{states.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                            <div className="mb-5"><label className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-1.5">Name</label><input className="w-full px-4 py-2.5 bg-white border border-brand-200 rounded-lg text-sm" value={districtForm.name} onChange={e => setDistrictForm({ ...districtForm, name: e.target.value })} required /></div>
                            <div className="mb-5"><label className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-1.5">Code</label><input className="w-full px-4 py-2.5 bg-white border border-brand-200 rounded-lg text-sm" value={districtForm.code} onChange={e => setDistrictForm({ ...districtForm, code: e.target.value })} required placeholder="e.g. LKO" /></div>
                            <div className="flex gap-3 justify-end mt-6 pt-6 border-t border-brand-100"><button type="button" className="px-4 py-2 border border-brand-200 text-brand-600 rounded-lg" onClick={() => setShowDistrictModal(false)}>Cancel</button><button type="submit" className="px-4 py-2 bg-brand-900 text-white rounded-lg font-medium shadow-sm">Create</button></div>
                        </form>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-brand-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={(e: any) => e.target === e.currentTarget && setShowBlockModal(false)}>
                    <div className="bg-white border border-brand-200 rounded-2xl w-full max-w-xl shadow-xl p-6">
                        ...
                        <form onSubmit={createBlock}>
                            <div className="mb-5"><label className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-1.5">District</label><select className="w-full px-4 py-2.5 bg-white border border-brand-200 rounded-lg text-sm cursor-pointer" value={blockForm.districtId} onChange={e => setBlockForm({ ...blockForm, districtId: e.target.value })} required><option value="">Select district</option>{districts.map(d => <option key={d.id} value={d.id}>{d.name}{d.state ? ` (${d.state.name})` : ''}</option>)}</select></div>
                            <div className="mb-5"><label className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-1.5">Name</label><input className="w-full px-4 py-2.5 bg-white border border-brand-200 rounded-lg text-sm" value={blockForm.name} onChange={e => setBlockForm({ ...blockForm, name: e.target.value })} required /></div>
                            <div className="mb-5"><label className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-1.5">Code</label><input className="w-full px-4 py-2.5 bg-white border border-brand-200 rounded-lg text-sm" value={blockForm.code} onChange={e => setBlockForm({ ...blockForm, code: e.target.value })} required placeholder="e.g. LKO-CHT" /></div>
                            <div className="flex gap-3 justify-end mt-6 pt-6 border-t border-brand-100"><button type="button" className="px-4 py-2 border border-brand-200 text-brand-600 rounded-lg" onClick={() => setShowBlockModal(false)}>Cancel</button><button type="submit" className="px-4 py-2 bg-emerald-600 text-white rounded-lg font-medium shadow-sm">Create</button></div>
                        </form>
                    </div>
                </div>
            )}
